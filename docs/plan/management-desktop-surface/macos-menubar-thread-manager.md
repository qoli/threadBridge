...
- tray menu 已新增 runtime assets 維護入口，用於在 bundled app 中清空 installed `runtime_assets/` 並從 bundle seed assets 重建
...
- tray 的 maintenance action 只額外保留 `Rebuild Runtime Assets`；它不應觸碰 `data/`、setup config 或 thread state
