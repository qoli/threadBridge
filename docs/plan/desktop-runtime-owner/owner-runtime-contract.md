...
owner 所管理的 backend plane today reality 與其長期收斂到 workspace-scoped child worker 的願景，則由 [app-server-ws-backend.md](app-server-ws-backend.md) 補充說明；本文件不重複展開 backend 子系統的完整 current-state 描述。
