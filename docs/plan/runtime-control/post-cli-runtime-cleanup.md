...
- Phase 2 已開始落地 launch / proxy cleanup：
  ...
  - `hcodex` launch contract 已收斂到 `hcodex_ws_url + one-shot launch_ticket`
  - 但 `hcodex-ws-bridge` 仍保留為必要的 compatibility boundary：launch URL 不能直接交給 Codex，且本地 reconnect 不能重撥同一個 launch ticket
  - local/TUI mirror intake 已開始從 proxy relay 熱路徑拆到獨立 app-server observer
  - `workspace_status` tests 已明確驗證 legacy `shared-runtime/*` / `local-session.json` 的 migrate/read/remove 邊界，且它們不會重新成為 canonical write path
...
- 若 canonical 與 legacy artifact 同時存在，`workspace aggregate` / `session snapshot` / `local TUI claim` 都一律優先 canonical path，legacy 只作 fallback
...
最明顯的是 [`workspace_status.rs`](../../../rust/src/workspace_status.rs)。
...
- [`workspace_status.rs`](../../../rust/src/workspace_status.rs#L15)
- [`workspace_status.rs`](../../../rust/src/workspace_status.rs#L23)
- [`workspace_status.rs`](../../../rust/src/workspace_status.rs#L91)
...
這個缺口已在 Phase 1 開始收斂：[`runtime_protocol.rs`](../../../rust/src/runtime_protocol.rs) 的 public runtime health 欄位已改成 `runtime_readiness`，同時保留既有 `ready` / `pending_adoption` / `degraded` / `unavailable` 狀態值。

具體可見：

- [`runtime_protocol.rs`](../../../rust/src/runtime_protocol.rs#L29)
- [`runtime_protocol.rs`](../../../rust/src/runtime_protocol.rs#L309)
- [`runtime_protocol.rs`](../../../rust/src/runtime_protocol.rs#L720)
- [`runtime_protocol.rs`](../../../rust/src/runtime_protocol.rs#L808)
...
目前 `hcodex` 的啟動主路徑已收斂為「取得 ingress launch URL，再透過 local bridge 轉成 Codex-safe remote endpoint」：

1. `ensure-hcodex-runtime`
2. Rust launch resolver 取得 `hcodex_ws_url` 與一次性 `launch_ticket`
3. `run-hcodex-session` 啟動 `hcodex-ws-bridge`
4. bridge 對 ingress 花掉一次 `launch_ticket`，並把本地 `ws://127.0.0.1:<port>/` 提供給 upstream Codex

具體可見：

- [`workspace.rs`](../../../rust/src/workspace.rs#L69)
- [`workspace.rs`](../../../rust/src/workspace.rs#L151)
- [`workspace.rs`](../../../rust/src/workspace.rs#L183)
- [`workspace.rs`](../../../rust/src/workspace.rs#L210)
- [`hcodex_runtime.rs`](../../../rust/src/hcodex_runtime.rs#L36)
- [`app_server_runtime.rs`](../../../rust/src/app_server_runtime.rs#L29)
...
- [`repository.rs`](../../../rust/src/repository.rs#L1533)

它仍接受舊欄位：

- `attachment_state = "local_handoff"`  
  [`repository.rs`](../../../rust/src/repository.rs#L1548)
...
- `workspace_status` 的 legacy `shared-runtime/*` 讀兼容、`local-tui-session.json` / legacy `local-session.json`、`LocalTuiSessionClaim`
...
- [`workspace_status.rs`](../../../rust/src/workspace_status.rs#L15)
- [`workspace_status.rs`](../../../rust/src/workspace_status.rs#L94)
- [`telegram_runtime/mod.rs`](../../../rust/src/telegram_runtime/mod.rs#L975)
- [`telegram_runtime/mod.rs`](../../../rust/src/telegram_runtime/mod.rs#L1001)
...
- 目前正式 launcher 只串接：
  ...
  - `run-hcodex-session`
- 舊的 `resolve_hcodex_launch.py` 與 `tui_proxy_base_ws_url` sideband 已退回歷史/compatibility 語境
- `hcodex-ws-bridge` 則仍是現行 `hcodex` launch contract 的一部分，直到 upstream Codex remote contract 真正放寬為止

具體可見：

- [`workspace.rs`](../../../rust/src/workspace.rs#L69)
- [`workspace.rs`](../../../rust/src/workspace.rs#L150)
- [`workspace.rs`](../../../rust/src/workspace.rs#L181)
- [`hcodex_runtime.rs`](../../../rust/src/hcodex_runtime.rs#L36)
- [`app_server_runtime.rs`](../../../rust/src/app_server_runtime.rs#L29)
...
- [`repository.rs`](../../../rust/src/repository.rs#L240)
- [`repository.rs`](../../../rust/src/repository.rs#L1535)
- [`codex.rs`](../../../rust/src/codex.rs#L179)
- [`telegram_runtime/mod.rs`](../../../rust/src/telegram_runtime/mod.rs#L97)
- [`bot_runner.rs`](../../../rust/src/bot_runner.rs#L24)

這些更接近：

- 非主路徑 fallback
- headless / compatibility 保留面
- 不是 today 對外文檔應描述的 supported operating model
...
- mirror intake observer 化細節  
  這由 [app-server-ws-mirror-observer.md](../app-server-observer/app-server-ws-mirror-observer.md) 處理
- Telegram adapter 的完整抽象化路線  
  這由 [runtime-transport-abstraction.md](runtime-transport-abstraction.md) 與 [telegram-adapter-migration.md](../telegram-adapter/telegram-adapter-migration.md) 處理
