...
```text
TELEGRAM_RUNNING_INPUT_POLICY=steer
```
...
- `v1` 已經達到使用者可感知的 busy gate 與 running input policy 行為
...
- 下一步應先把 Telegram running input policy 固定為 `reject | queue | steer`，預設 `steer`
