...
- `hcodex` 已具備過渡性的本地 self-heal：
  - 如果 `./.threadbridge/state/app-server/current.json` 指到 stale ws endpoint
  - `hcodex` 目前仍會先在本機補拉 shared daemon / TUI proxy，再啟動 `codex --remote`

目前仍未完成：

- handoff 所依賴的 workspace `ws` runtime 正式 owner 仍未收斂
...
- 若 `current.json` 已 stale，`hcodex` 目前仍會先做 self-heal，再把活的 endpoint 交給 `codex --remote`
- 但這個 self-heal 應理解成 owner 未正式收斂前的 fallback，不是長期 runtime ownership 模型

### 3. Runtime Ownership 現況

- bot 端目前已經是 shared runtime 的 client，但還不是可靠的長壽命 owner
- `hcodex` 目前也還保留補拉 runtime 的能力，但它不應成為 handoff `ws` runtime 的正式 owner
...
- 所以目前的 operational reality 是：
  ...
  - handoff continuity 所依賴的 workspace `ws` runtime，仍缺少單一正式 owner
...
- 需要把 shared daemon / TUI proxy / handoff `ws` runtime 的長壽命 owner 正式定下來
- 目前更合理的方向是：
  - `hcodex` self-heal 保留為本地 fallback
  - 後續由 macOS menubar 常駐進程接手 workspace `ws` runtime ownership 與 health management
...
1. 收斂 shared daemon / TUI proxy / handoff `ws` runtime 的長壽命 owner 模型。
