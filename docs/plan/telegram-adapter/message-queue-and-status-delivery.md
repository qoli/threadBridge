...
- `telegram -> codex` 路徑上的 draft message heartbeat 已存在
- local/TUI mirror `preview_text` 已開始保留 upstream `turn_id`
- local/TUI mirror draft 已開始用 shared `DeliveryBusCoordinator` 的 turn-bound claim 做單 turn 去重
- workspace outbox item 已開始帶有最小 `surface` 欄位，預設為 `content`
...
- 完整且正式的 content / draft / status / edit delivery 規格
- direct path 與 mirror path 的統一 draft owner / heartbeat contract
...
- TUI mirror draft 的多 writer 衝突已確認與 `turn_id` 丟失有關
...
- local/TUI mirror draft 現在已透過 `turn_id` 傳遞與 `PreviewDraft` turn claim 做單 turn 去重
- 但 mirror draft 的 writer 與 heartbeat 目前仍掛在 `status_sync` poll consumer 上，這是 adapter 內部尚未收斂的責任債務
- 後續應把「誰擁有某個 Telegram draft surface」收成單一 owner，而不是讓 mirror consumer 本身兼任 writer / heartbeater
...
- workspace outbox `text` 目前也已開始顯式區分 `surface`，但仍未形成完整的統一 lane / ordering contract
...
- 大小門檻應集中在 Telegram adapter config，而不是長期散落在各個 callsite
- 檔案超限時不要盲送再等 API 失敗
...
- preflight 已開始存在
- `photo` 超限時可改走 `document`
- oversized `document` / final reply attachment 已開始回到 warning / notice 路徑
- 但門檻仍未抽成集中 config，且更完整的 artifact fallback 仍未定稿
...
- 不重試成普通 message
- 不阻塞後續 final reply
- 若 draft 預期存活超過單次更新窗口，delivery 層應主動續發 heartbeat；不能假設 mirror 只要持續輸出內容就足以保活
...
- 目前會回到明確 warning notice
- 但更理想的 artifact / URL fallback 仍未收斂
...
- workspace outbox `photo` / `document` 已有最小 preflight 與 fallback
- 但其他 artifact 類 payload 仍未完全掛回同一份 delivery 契約
...
- 這些語義只有部分進入代碼
- workspace outbox / final reply 的 size preflight 與 fallback 已開始落地
- `codex mirror` draft heartbeat 已存在，但目前仍掛在 `status_sync` poll consumer 上，draft owner 責任尚未收斂
- queue、完整 control lifecycle、artifact 類型與集中化 config 仍未完全收斂
