...
- `shared-runtime` 在文檔上被明確定位為 workspace-local observation surface，而不是 canonical authority
...
2. 補齊 `workspace-runtime-surface` 與相關文檔，把 `.threadbridge/state/shared-runtime/*` 明確寫成 workspace-local observation surface。
