...
- Rust bot and repository layer own workspace state such as `metadata.json`, `conversations.jsonl`, `summary.md`, `pending-image-batch.json`, `images/source/`, and `images/analysis/`.
- Workspace bootstrap logic owns the presence of `AGENTS.md`, `bin/`, and `tool_requests/` in each workspace.
